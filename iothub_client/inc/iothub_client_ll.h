...
typedef struct IOTHUB_CLIENT_CORE_LL_HANDLE_DATA_TAG* IOTHUB_CLIENT_LL_HANDLE;
    ...
    *           @b NOTE: The application behavior is undefined if the user calls
    *           the ::IoTHubClient_LL_Destroy function from within any callback.
    *           Usage: user has to call IoTHubClient_LL_SetDeviceTwinCallback API
    *           before calls IoTHubClient_LL_GetDeviceTwin to have the twin callback function set up
    *
    * @return   IOTHUB_CLIENT_OK upon success or an error code upon failure.
    */
    MOCKABLE_FUNCTION(, IOTHUB_CLIENT_RESULT, IoTHubClient_LL_GetDeviceTwin, IOTHUB_CLIENT_LL_HANDLE, iotHubClientHandle);
